...
async function getAccessToken() {
  ...
  const accessToken = String(jsonRes?.access_token || "");
  const expiresIn = Number(jsonRes?.expires_in || 0);
...
async function buildGameDetail(gameId) {
...
function getRouteRemainder(pathValue) {
  const path = String(pathValue || "").split("?")[0];
  const normalized = path.replace(/^\/+|\/+$/g, "");
  let remainder = normalized
    .replace(/^api\/igdb\/?/, "")
    .replace(/^\.netlify\/functions\/igdb\/?/, "");
  if (!remainder && normalized.endsWith("igdb")) remainder = "";
  return remainder;
}

export async function handler(event) {
  try {
    const route = getRouteRemainder(event.path);
    const qs = event.queryStringParameters || {};

    if (route === "" || route === "games") {
      if (qs.id) {
        const gameId = Number(qs.id);
        if (!Number.isFinite(gameId) || gameId <= 0) {
          return json(400, { message: "Invalid game id." });
        }
        const detail = await buildGameDetail(gameId);
        if (!detail) return json(404, { message: "Game not found." });
        return json(200, detail);
      }

      const page = clampInt(qs.page, 1, 100000, 1);
      const pageSize = clampInt(qs.page_size, 1, 50, 20);
      const offset = (page - 1) * pageSize;
      const ordering = sortClauseFromOrdering(qs.ordering);
      const search = String(qs.search || "").trim().slice(0, 100);
      const { startUnix, endUnix } = parseDatesRange(qs.dates);
      const genreIds = await resolveGenreIds(qs.genres);
    ...
    if (route === "genres") {
      const pageSize = clampInt(qs.page_size, 1, 100, 50);
      const cache = await ensureGenreCache();
      return json(200, {
    ...
    if (route.startsWith("games/")) {
      const idPart = route.slice("games/".length);
      const gameId = Number(idPart);
      if (!Number.isFinite(gameId) || gameId <= 0) {
        return json(400, { message: "Invalid game id." });
      }
      const detail = await buildGameDetail(gameId);
    ...
    return json(500, { message: "IGDB function failed.", error: String(error?.message || error) });
  }
}
