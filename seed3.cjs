...
async function renameSvgToPng() {
  const brandsToFix = [
    { name: 'Chipotle', oldPath: 'food_brands/chipotle-com.svg', newPath: 'food_brands/chipotle-com.png' },
    { name: 'Buffalo Wild Wings', oldPath: 'food_brands/buffalowildwings-com.svg', newPath: 'food_brands/buffalowildwings-com.png' }
  ];
  ...
  for (const brand of brandsToFix) {
    try {
      ...
      const { error: dbError } = await supabase.from('brands').update({ logo_url: brand.newPath }).eq('name', brand.name);
      if (dbError) {
