## Goal
Fix list saving so adding/removing items from lists (all media types: books, music, games, movies, TV, anime, sports, travel, etc.) produces zero network or runtime errors.
...
**Root cause**: All save paths used `insert()` against unique indexes. Duplicate clicks caused 409 Conflict. The 409 was swallowed in JS but the network error still fired.

**Fix**: `insert()` → `upsert({ onConflict: …, ignoreDuplicates: true })` in all save paths.
...
| `games.html` `toggleDefaultList()` | Already used `upsert()` (was correct) |
| `tvshows.html` | Already used `upsert()` (was correct) |
| `animes.html` | Already used `upsert()` (was correct) |
| `movies.html` | Already used `upsert()` (was correct) |
| `js/pages/game.js` | `insert()` → `upsert()` |
| `js/components/list-modal.js` | `insert()` → `upsert()` (affects ALL media types) |
| `js/pages/sports.js` | `insert()` → `upsert()` |
| `js/travel-home-directory.js` | `insert()` → `upsert()` |
| `js/list-utils.js` `addItemToList()` | `insert()` → `upsert()` (split into 2 conflict paths: default vs custom lists) |
| `js/list-utils.js` `saveCustomListChanges()` | `insert()` → `upsert()` (batch insert) |

## Phase 2 — Fix FK 23503 (missing parent record) — DONE
**Root cause**: Some save paths upsert into items tables WITHOUT first ensuring the parent record exists. FK constraints like `book_list_items.book_id → books(id)` or `music_list_items.track_id → tracks(id)` (NOT NULL, not deferrable).
...
**Fix**: Added minimal parent-record upsert before items upsert in write paths:

| File | What changed |
|---|---|
| `js/pages/game.js` | Added `ensureGameInSupabase()` call before items upsert |
...
**Hypothesis**: Race condition in the adapter's `ensureClient()`. The adapter may reject the bridge's client if `__zo2yAuthListenersBound` is not set and `__ZO2Y_ENSURE_SUPABASE_CLIENT` is available. The adapter then creates its own client via `ensureSharedSupabaseClient` which uses `attachClientListeners`. If this shared-client creation path fails or returns null, `ensureClient()` falls through to `ZO2Y_AUTH.ensureClient()` which might return a different object that does not set the `apikey` header.
