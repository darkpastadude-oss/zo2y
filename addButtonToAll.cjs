...
window.addEventListener('DOMContentLoaded', function() {
    ...
    script.innerHTML = \`
        ...
        console.log('✅ Supabase initialized via ES modules');
...
class RestaurantListManager {
    constructor() {
        ...
        this.userLists = [];
        this.customLists = [];
        console.log('🎯 RestaurantListManager created for ID:', this.restaurantId);
        this.init();
    }
    ...
    async checkAuth() {
        try {
            ...
            console.log('🔐 Auth result:', this.currentUser ? 'User logged in' : 'No user');
        } catch (error) {
    ...
    injectListMenu() {
        console.log('🎯 Injecting list menu...');
        ...
        const existingStyles = document.querySelector('style[data-list-manager]');
        if (existingStyles) {
        ...
        styleSheet.setAttribute('data-list-manager', 'true');
        ...
        console.log('✅ List Manager FAB Added to page!');
    }
...
try {
    ...
    files.forEach((file) => {
        try {
            ...
            const existingScriptRegex = /<!-- List Manager Button[\s\S]*?<\/script>/gi;
            content = content.replace(existingScriptRegex, '');

            // Check if file has proper HTML structure
            if (content.includes('</body>')) {
                // File has proper body tag - insert before </body>
                content = content.replace('</body>', `${scriptCode}</body>`);
                console.log(`   ✅ Added premium FAB to ${file} (before </body>)`);
                processedCount++;
            } else if (content.includes('</html>')) {
                // File has html tag but no body tag - insert before </html>
                content = content.replace('</html>', `${scriptCode}</html>`);
                console.log(`   ✅ Added premium FAB to ${file} (before </html>)`);
                processedCount++;
            } else {
                // No proper closing tags - just append to the end
                content += scriptCode;
                console.log(`   ✅ Added premium FAB to ${file} (appended to end)`);
                processedCount++;
            }
    ...
    if (processedCount > 0) {
        console.log('\n✨ Button should now appear in bottom-right corner!');
    }
