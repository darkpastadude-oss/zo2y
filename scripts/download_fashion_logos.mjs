...
async function downloadCommonsLogo(filename) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (buf.length > 300) {
        return { buffer: buf, ext: filename.endsWith('.png') ? 'png' : 'svg' };
      }
    }
  } catch(e) {}
  return null;
...
async function run() {
  ...
  console.log(`Downloading real vector logos for ${items.length} fashion brands...`);
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const commonsFile = COMMONS_MAP[item.name];
    let logoData = commonsFile ? await downloadCommonsLogo(commonsFile) : null;

    if (!logoData) {
      // Fallback download via Unavatar / Clearbit
      const fallbackUrl = `https://unavatar.io/${domain}?fallback=false`;
      try {
        const res = await fetch(fallbackUrl, { headers: { 'User-Agent': 'Mozilla/5.0' } });
        if (res.ok) {
          const buf = Buffer.from(await res.arrayBuffer());
          if (buf.length > 1000) logoData = { buffer: buf, ext: 'png' };
        }
      } catch(e) {}
    }

    if (logoData) {
      const filePath = path.join(fashionDir, `${cleanSlug}.${logoData.ext}`);
      fs.writeFileSync(filePath, logoData.buffer);
      console.log(`[${i + 1}/${items.length}] ✓ SAVED REAL LOCAL ASSET: ${item.name} -> ${filePath} (${logoData.buffer.length} bytes)`);
    } else {
  ...
  console.log('\nFinished downloading real local fashion logos!');
}
