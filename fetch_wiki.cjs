...
async function getImageUrl(query) {
    try {
        const url = `https://en.wikipedia.org/w/api.php?action=query&generator=search&gsrsearch=${encodeURIComponent(query)}&gsrnamespace=6&gsrlimit=10&prop=imageinfo&iiprop=url|size&iiurlwidth=1200&format=json&origin=*`;
        ...
        if (data.query && data.query.pages) {
            ...
            for (const p of pages) {
                if (p.imageinfo && p.imageinfo[0]) {
                    ...
                    // Strict filtering
                    const badWords = /(logo|svg|pdf|building|store|shop|hq|headquarters|office|exterior|founder|portrait|ceo|map|chart)/i;
                    if (!badWords.test(fileUrl) && !badWords.test(p.title)) {
                        return fileUrl;
                    }
                }
            }
...
async function run() {
    ...
    fs.writeFileSync('brand_wiki_dict.json', JSON.stringify(dict, null, 2));
    console.log('done');
}
