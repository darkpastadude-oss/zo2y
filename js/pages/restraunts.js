(function() {
        ...
        if (window.supabase && window.supabase.createClient) {
          supabase = window.supabase.createClient(SUPABASE_URL, SUPABASE_KEY, {
            auth: {
              ...
              // OAuth callback is handled on auth-callback.html.
      ...
      async function getVerifiedRestaurantUser() {
      ...
      async function initializeAuth() {
        ...
        try {
          const user = await getVerifiedRestaurantUser();
          ...
          const profileReady = await ensureUserProfileExists(user);
      ...
      function updateModalButtons(restaurantId) {
        ...
        if (visitedBtn) {
          visitedBtn.innerHTML = `<i class="fas fa-clapperboard"></i><span>${status.visited ? 'Remove from Visited' : 'Mark as Visited'}</span>`;
