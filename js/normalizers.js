(function () {
  ...
  function extractArtistNames(raw) {
    const ra = raw.artists || (raw.artist ? [raw.artist] : []);
    const list = Array.isArray(ra) ? ra : [ra];
    return list
      .map(function (a) {
  ...
  function extractImage(raw) {
    return raw.image || raw.cover ||
      (raw.images && raw.images.length > 0 ? (raw.images[0].url || raw.images[0]) : '') ||
      (raw.album && raw.album.images && raw.album.images.length > 0 ? (raw.album.images[0].url || raw.album.images[0]) : '') ||
      '';
  }
  ...
  window.normalizeBook = function normalizeBook(raw) {
    ...
    const thumbnail = imageLinks.thumbnail || imageLinks.smallThumbnail || data.image || data.cover || '';
    ...
    const publishedDate = data.publishedDate || data.releaseDate || (data.year ? String(data.year) : '') || '';
  ...
  window.normalizeAlbum = function normalizeAlbum(raw) {
    if (!raw) return null;
    const artistStr = extractArtistNames(raw);
  ...
  window.normalizeTrack = function normalizeTrack(raw) {
    ...
    return {
      ...
      albumName: String(raw.albumName || raw.album_name || (raw.album ? raw.album.name : '') || '').trim(),
      albumId: String(raw.albumId || raw.album_id || (raw.album ? raw.album.id : '') || '').trim(),
