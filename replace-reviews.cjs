// fresh-add-reviews.js
const fs = require('fs');
const path = require('path');

// Function to add reviews section to a single file
function addReviewsToFile(filePath) {
    try {
...
<style>
.reviews-container { max-height: 500px; overflow-y: auto; padding-right: 10px; margin-bottom: 20px; }
.review { border-bottom: 1px solid var(--nav-shadow); padding: 20px 0; position: relative; }
...
.reviewer-name { font-weight: 600; color: var(--text); margin: 0; flex: 1; padding-right: 100px; }
...
.rating-input { display: flex; align-items: center; gap: 10px; margin-bottom: 15px; flex-wrap: wrap; }
...
.rating-text { font-size: 0.9rem; color: var(--text2); min-width: 100px; font-weight: 500; }
...
.zo2y-toast.show { transform: translateX(0); }
.zo2y-toast.success { border-left-color: #10b981; }
.zo2y-toast.error { border-left-color: #ef4444; }
.zo2y-toast.warning { border-left-color: #f59e0b; }
.zo2y-toast.info { border-left-color: #3b82f6; }
...
@media (max-width: 768px) {
  ...
  .reviewer-name { padding-right: 0; margin-bottom: 0; }
  .review-rating { position: static; }
  .review-actions { position: static; align-self: flex-end; }
  .rating-input { flex-direction: column; gap: 10px; }
  .zo2y-toast { top: 10px; right: 10px; left: 10px; max-width: none; }
}
</style>
`;
        ...
        const reviewsScript = `
<!-- REVIEWS SYSTEM SCRIPT -->
<script>
// Simple check to prevent multiple initializations
if (!window.reviewSystemInitialized) {
  ...
  class RestaurantReviews {
    constructor(restaurantId) {
      ...
      console.log('🔄 Review system initializing for restaurant:', restaurantId);
      this.init();
    }
    ...
    async init() {
      try {
        ...
        console.error('❌ Review system init failed:', error);
      }
    }
    ...
    async loadReviews() {
      ...
      try {
        ...
        await this.displayReviews(reviews || []);
      } catch (error) {
            ...
            <div class="review-header">
              ...
              \${this.currentUser && review.user_id === this.currentUser.id ? \`
                <div class="review-actions">
                  <button class="delete-review" onclick="window.currentReviews.deleteReview(\${review.id})">
  ...
  function initReviews() {
    if (window.supabase) {
      window.currentReviews = new RestaurantReviews(${restaurantId});
    } else {
...
`;
        ...
        // STRATEGY: Just add the reviews section before the footer
        const footerIndex = content.indexOf('<footer');
        const bodyCloseIndex = content.indexOf('</body>');
        
        let insertIndex = footerIndex !== -1 ? footerIndex : bodyCloseIndex;
        
        if (insertIndex !== -1) {
            // Insert reviews section
            ...
            console.log(`✅ Added reviews system to: ${filePath}`);
            return true;
        } else {
...
function processAllRestaurantFiles() {
    ...
    files.forEach(file => {
        if (file.endsWith('.html')) {
            ...
            if (addReviewsToFile(filePath)) {
                processedCount++;
            }
        }
    });

    console.log(`\n🎉 Successfully added reviews to ${processedCount} files`);
}
